...
#include "CustomPropertyPanelSub.h"

struct CustomPropertyPanelSub::SectionComponent : public Component
{
	...
	void setOpen(const bool open)
	{
		if (isOpen != open)
		{
			...
			if (CustomPropertyPanelSub* const pp = findParentComponentOfClass<CustomPropertyPanelSub>()) {
				pp->resized();
	...
	bool isOpenState;
	Colour bordercolor;
	CustomPropertyPanelSub parent;
	int index;
	JUCE_DECLARE_NON_COPYABLE(SectionComponent)
};

//==============================================================================
struct CustomPropertyPanelSub::PropertyHolderComponent : public Component
{
...
//==============================================================================
CustomPropertyPanelSub::CustomPropertyPanelSub() :PropertyComponent("Name")
{
	init();
}
CustomPropertyPanelSub::CustomPropertyPanelSub(int _height) : height(_height), PropertyComponent("Name", height)
{
	init();
}
CustomPropertyPanelSub::CustomPropertyPanelSub(const String& name, int _height) : height(_height), PropertyComponent(name, height)
{
	init();
}

void CustomPropertyPanelSub::init()
{
...
CustomPropertyPanelSub::~CustomPropertyPanelSub()
{
	listeners.clear();
	clear();
...
//==============================================================================
void CustomPropertyPanelSub::paint(Graphics& g)
{
	if (isEmpty())
	{
...
void CustomPropertyPanelSub::resized()
{
	viewport.setBounds(getLocalBounds());
	updatePropHolderLayout();
}

void CustomPropertyPanelSub::valueChanged(Value &)
{

}

void CustomPropertyPanelSub::callChangeListeners( )
{

	Component::BailOutChecker checker(this);
	listeners.callChecked(checker, &CustomPropertyPanelSubListener::selectedChanged, this, indexActive);  // (can't use Label::Listener due to idiotic VC2005 bug)
}



void CustomPropertyPanelSub::addListener(CustomPropertyPanelSubListener * listener)
{
	listeners.add(listener);
}

void CustomPropertyPanelSub::removeListener(CustomPropertyPanelSubListener * listener)
{
	listeners.remove(listener);
}

//==============================================================================
void CustomPropertyPanelSub::clear()
{
	if (!isEmpty())
	{
...
bool CustomPropertyPanelSub::isEmpty() const
{
	return propertyHolderComponent->sections.size() == 0;
}

int CustomPropertyPanelSub::getTotalContentHeight() const
{
	return propertyHolderComponent->getHeight();
}


void CustomPropertyPanelSub::addProperties(const Array<PropertyComponent*>& newProperties,const int titleHeight)
{
	if (isEmpty())
		repaint();
...
void CustomPropertyPanelSub::addSection(const String& sectionTitle,
	const Array<PropertyComponent*>& newProperties,
...
void CustomPropertyPanelSub::updatePropHolderLayout() const
{
...
void CustomPropertyPanelSub::refreshAll() const
{
	propertyHolderComponent->refreshAll();
}

void CustomPropertyPanelSub::refresh()
{
	refreshAll();
}
...
//==============================================================================
StringArray CustomPropertyPanelSub::getSectionNames() const
{
	StringArray s;
...
bool CustomPropertyPanelSub::isSectionOpen(const int sectionIndex) const
{
...
void CustomPropertyPanelSub::setSectionOpen(const int sectionIndex, const bool shouldBeOpen)
{
...
void CustomPropertyPanelSub::setSectionEnabled(const int sectionIndex, const bool shouldBeEnabled)
{
	if (SectionComponent* s = propertyHolderComponent->getSectionWithNonEmptyName(sectionIndex))
		s->setEnabled(shouldBeEnabled);
}

void CustomPropertyPanelSub::removeSection(int sectionIndex)
{
	if (SectionComponent* s = propertyHolderComponent->getSectionWithNonEmptyName(sectionIndex))
	{
...
//==============================================================================
XmlElement* CustomPropertyPanelSub::getOpennessState() const
{
...
void CustomPropertyPanelSub::restoreOpennessState(const XmlElement& xml)
{
...
//==============================================================================
void CustomPropertyPanelSub::setMessageWhenEmpty(const String& newMessage)
{
...
void CustomPropertyPanelSub::setCollapseAll()
{
	propertyHolderComponent->collapseAll();
}

const String& CustomPropertyPanelSub::getMessageWhenEmpty() const noexcept
{
